...
//    FragColor = vec4((texture(gNormal, TexCoords).xyz + vec3(1.0)) / 2.0f, 1.0f);
    vec2 MotionVector = texture(gPosition, TexCoords).xy / 4.0;
    vec4 Color = vec4(0.0);

    vec2 TexCoord = TexCoords;
    Color += texture(gNormal, TexCoord);
    
    // Color += texture(gNormal, TexCoord) * 0.4;
    // TexCoord -= MotionVector;
    // Color += texture(gNormal, TexCoord) * 0.3;
    // TexCoord -= MotionVector;
    // Color += texture(gNormal, TexCoord) * 0.2;
    // TexCoord -= MotionVector;
    // Color += texture(gNormal, TexCoord) * 0.1;
