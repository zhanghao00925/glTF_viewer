...
{
    // velocity
    vec3 NDCPos = (ss_pos / ss_pos.w).xyz;
    vec3 PrevNDCPos = (pre_ss_pos / pre_ss_pos.w).xyz;
    vec2 velocity = (NDCPos - PrevNDCPos).xy * vec2(1920.0, 1080.0) * 0.5;
    float velocity_length = length(velocity);
    velocity = max(0.49, min(velocity_length, 20))  * normalize(velocity);
    gPosition.xy = velocity;
