...
layout (location = 0) out vec4 envCubemap;
...
// ----------------------------------------------------------------------------
void main() {
    vec3 albedo     = pow(baseColor * texture(baseColorMap, TexCoords * baseColorMapScale.xy).rgb, vec3(2.2));
    float metallic  = metallic * texture(metallicMap, TexCoords).r;
    float roughness = 1.0f - glossiness * texture(glossinessMap, TexCoords).r;
    float ao        = 1.0f;
    ...
    envCubemap = vec4(color , 1.0);
    envCubemap = vec4(1.0, 0.0, 0.0 , 1.0);
}
