...
void SkinnedMesh::ReadNodeHeirarchy(Shader shader, float AnimationTime, const aiNode* pNode, const mat4& ParentTransform)
{    
    ...
    if (pNodeAnim) {
        ...
        mat4 ScalingM;
        ScalingM = glm::scale(mat4(1.0f), vec3(Scaling.x, Scaling.y, Scaling.z));
        ...
        mat4 TranslationM;
        TranslationM = glm::translate(mat4(1.0f), vec3(Translation.x, Translation.y, Translation.z));
    ...
    mat4 local_model = m_GlobalInverseTransform * GlobalTransformation;
    shader.setMat4("localModel", local_model);

    if (m_BoneMapping.find(NodeName) != m_BoneMapping.end()) {
        uint BoneIndex = m_BoneMapping[NodeName];
        m_BoneInfo[BoneIndex].FinalTransformation = m_GlobalInverseTransform * GlobalTransformation * m_BoneInfo[BoneIndex].BoneOffset;
        shader.setMat4(string("bones[") + to_string(BoneIndex) + "]", m_BoneInfo[BoneIndex].FinalTransformation);
    }

    for (int i = 0; i < pNode->mNumMeshes; i++) {
        auto entry_index = pNode->mMeshes[i];
        if (m_pScene->mMeshes[entry_index]->HasBones()) {
        ...
        glBindVertexArray(m_VAO);
        glDrawElementsBaseVertex(GL_TRIANGLES,
                                 m_Entries[entry_index].NumIndices,
                                 GL_UNSIGNED_INT,
                                 (void *) (sizeof(uint) * m_Entries[entry_index].BaseIndex),
                                 m_Entries[entry_index].BaseVertex);
        glBindVertexArray(0);
    }
    
    for (uint i = 0 ; i < pNode->mNumChildren ; i++) {
        ReadNodeHeirarchy(shader, AnimationTime, pNode->mChildren[i], GlobalTransformation);
    }
}


void SkinnedMesh::BoneTransform(Shader shader, float TimeInSeconds, vector<mat4>& Transforms)
{
    shader.Use();
    mat4 Identity = mat4(1.0f);
    // Identity.InitIdentity();
    
    float TicksPerSecond = (float)(m_pScene->mAnimations[0]->mTicksPerSecond != 0 ? m_pScene->mAnimations[0]->mTicksPerSecond : 25.0f) * 50.0f;
    float TimeInTicks = TimeInSeconds * TicksPerSecond;
//    float AnimationTime = fmod(TimeInTicks, (float)m_pScene->mAnimations[0]->mDuration);

    float AnimationTime = fmod(TimeInTicks, (float)m_pScene->mAnimations[0]->mDuration-1292);
    AnimationTime += 1292;

    ReadNodeHeirarchy(shader, AnimationTime, m_pScene->mRootNode, Identity);
